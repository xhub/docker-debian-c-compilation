...
RUN GCC_VER=15; LLVM_VER=21; \
    ...
    echo "deb http://apt.llvm.org/unstable/ llvm-toolchain-${LLVM_VER} main" >> /etc/apt/sources.list.d/llvm.list && \
    echo "deb-src http://apt.llvm.org/unstable/ llvm-toolchain-${LLVM_VER} main" >> /etc/apt/sources.list.d/llvm.list && \
    apt-get update --yes && \
    apt-get install --yes \
      build-essential \
      bzip2 \
